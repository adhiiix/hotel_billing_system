...
def reset():
    ...
    textsarbath.config(state=DISABLED)
...
def receipt():
    ...
    if costoffoodvar.get() != '' or costofcakesvar.get() != '' or costofdrinksvar.get() != '':
        ...
        textReceipt.insert(END,'Bill Receipt Ref:\t\t'+billnumber+'\t\t'+date+'\n')
        ...
        if e_chapati.get() != '0':
            textReceipt.insert(END, f'Chapati:\t\t\t{int(e_chapati.get()) * 50}\n\n')

        if e_briyani.get() != '0':
            textReceipt.insert(END, f'Briyani:\t\t\t{int(e_briyani.get()) * 100}\n\n')
        ...
        if e_idli.get() != '0':
            textReceipt.insert(END, f'Idli:\t\t\t{int(e_idli.get()) * 120}\n\n')
        ...
        if e_sarbath.get() != '0':
            textReceipt.insert(END, f'Sarbath:\t\t\t{int(e_sarbath.get()) * 30}\n\n')
        ...
        textReceipt.insert(END, f'Total\t\t\t{subtotalofItems}Rs\n\n')
        textReceipt.insert(END, f'Service Tax\t\t\t{500}Rs\n\n')
        textReceipt.insert(END, f'Total Cost\t\t\t{subtotalofItems+500}Rs\n\n')
        textReceipt.insert(END,'***************************************************************\n')
...
def save_to_database():
    try:
        ...
        if connection.is_connected():
            ...
            receipt_data = textReceipt.get(1.0, END)

            
            insert_query = "INSERT INTO orders (receipt_data) VALUES (%s)"

           
            cursor.execute(insert_query, (receipt_data,))
            ...
            messagebox.showinfo('Information', 'Order receipt saved to the database')

    except Error as e:
        print(f"Error: {e}")
        messagebox.showerror('Error', 'Failed to save order receipt to the database')
...
def sarbath():
    if var13.get() == 1:
        textsarbath.config(state=NORMAL)
        textsarbath.focus()
        textsarbath.delete(0, END)
    elif var13.get() == 0:
        textsarbath.config(state=DISABLED)
        e_sarbath.set('0')
...
topFrame=Frame(root,bd=10,relief=SOLID,bg='AntiqueWhite2')
topFrame.pack(side=TOP)

labelTitle=Label(topFrame,text='Hotel Billing System',font=('algerian',30,'bold'),fg='blue',bd=9,
                 bg='AntiqueWhite2',width=51)
labelTitle.grid(row=0,column=0)

#frames

menuFrame=Frame(root,bd=10,relief=RIDGE,bg='chocolate1')
menuFrame.pack(side=RIGHT)

costFrame=Frame(menuFrame,bd=4,relief=RIDGE,bg='darkolivegreen1',pady=10)
costFrame.pack(side=BOTTOM)

foodFrame=LabelFrame(menuFrame,text='Food',font=('arial',19,'bold'),bd=10,relief=RIDGE,fg='yellow4',)
...
rightFrame=Frame(root,bd=15,relief=RIDGE,bg='mediumspringgreen')
rightFrame.pack(side=RIGHT)

calculatorFrame=Frame(rightFrame,bd=1,relief=RIDGE,bg='black')
calculatorFrame.pack()

recieptFrame=Frame(rightFrame,bd=4,relief=RIDGE,bg='khaki2')
recieptFrame.pack()

buttonFrame=Frame(rightFrame,bd=3,relief=RIDGE,bg='lightseagreen')
buttonFrame.pack()
...
chapati=Checkbutton(foodFrame,text='Chapati',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var4
                  ,command=chapati)
chapati.grid(row=3,column=0,sticky=W)
...
dosa=Checkbutton(foodFrame,text='Dosa',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var6
                   ,command=dosa)
dosa.grid(row=5,column=0,sticky=W)

idli=Checkbutton(foodFrame,text='Idli',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var7,
                   command=idli)
idli.grid(row=6,column=0,sticky=W)

briyani=Checkbutton(foodFrame,text='Briyani',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var8
                   ,command=briyani)
briyani.grid(row=7,column=0,sticky=W)
...
textchapati.grid(row=3, column=1)
...
sarbath=Checkbutton(drinksFrame,text='Sarbath',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var13
                     ,command=sarbath)
sarbath.grid(row=3,column=0,sticky=W)
...
textsarbath = Entry(drinksFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_sarbath)
textsarbath.grid(row=3, column=1)
...
buttonSave=Button(buttonFrame,text='Print',font=('arial',14,'bold'),fg='white',bg='red4',bd=3,padx=5
                  ,command=save)
buttonSave.grid(row=0,column=2)

buttonReset=Button(buttonFrame,text='Reset',font=('arial',14,'bold'),fg='white',bg='red4',bd=3,padx=5,
                   command=reset)
buttonReset.grid(row=0,column=5)

save_button = Button(buttonFrame,text='Save',font=('arial',14,'bold'),fg='white',bg='red4',bd=3,padx=5,
